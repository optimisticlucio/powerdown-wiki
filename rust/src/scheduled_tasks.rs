...
pub fn initiate_scheduled_tasks(state: ServerState) {
    ...
    tokio::spawn(run_periodically(state.clone(), sql_backup::run_backup_processes, MINUTE_DURATION.saturating_mul(10)));
    tokio::spawn(run_periodically(state.clone(), clean_temp_db_entries, MINUTE_DURATION.saturating_mul(15)));
}
