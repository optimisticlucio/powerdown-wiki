...
const CHARACTER_THUMBNAIL_COMPRESSION_SETTINGS: utils::file_compression::LossyCompressionSettings =
...
const CHARACTER_IMAGE_COMPRESSION_SETTINGS: utils::file_compression::LossyCompressionSettings =
    utils::file_compression::LossyCompressionSettings {
        max_width: Some(500),
        max_height: Some(500),
        quality: 90,
...
const CHARACTER_LOGO_COMPRESSION_SETTINGS: utils::file_compression::LossyCompressionSettings =
    utils::file_compression::LossyCompressionSettings {
        max_width: Some(100),
        max_height: Some(100),
        quality: 90,
...
pub async fn add_character(
    ...
    match posting_step {
        ...
        PostingSteps::UploadMetadata(mut recieved_page_character) => {
            ...
            let character_id: i32 = db_connection
                .query_one(&creation_query, &values)
                .await
                .map_err(|err| {
                    eprintln!(
...
fn sanitize_recieved_page_character(
    recieved_page_character: &mut PageCharacter,
    state: &ServerState,
) {
    ...
    recieved_page_character.page_img_key =
        utils::clean_passed_key(&recieved_page_character.page_img_key, state).unwrap_or_default();
    recieved_page_character.base_character.thumbnail_key =
        utils::clean_passed_key(&recieved_page_character.base_character.thumbnail_key, state)
            .unwrap_or_default();

    // Make sure none of the lists have empty values in them
    recieved_page_character.subtitles = recieved_page_character
        .subtitles
        .iter()
        .filter_map(|subtitle| {
