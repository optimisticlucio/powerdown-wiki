...
pub async fn get_discord_link(db_connection: &Object<Manager>) -> Option<String> {
    const GET_DISCORD_LINK_QUERY: &str = "SELECT item_value FROM arbitrary_value WHERE item_key='discord_invite_url';";

    // Assume `discord_invite_url` key exists
    let resulted_row = db_connection.query_one(GET_DISCORD_LINK_QUERY, &[])
            .await
            .map_err(|err|{
                eprintln!("[GET DISCORD LINK] Failed reading `discord_invite_url`! Returning None. Err: {:?}", err)
            })
            .ok()?;
