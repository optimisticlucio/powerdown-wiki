        ...
        match trimmed_option {
            ...
            "3" => {
                ...
                loop {
                    ...
                    let chosen_file = all_story_paths.iter().find(|path| {
                        path.file_name()
                            .unwrap_or_default()
...
async fn import_given_story(
    _: &Path,
    story_file_path: &Path,
    server_url: &Url,
) -> Result<Response, String> {
    // Read and parse file
    let file_contents = fs::read_to_string(story_file_path)
    ...
    reqwest::Client::new()
        ...
        .send()
        .await
        .map_err(|err| format!("Info Send Err: {err}"))
}
