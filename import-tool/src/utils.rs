    ...
    Fut: Future<Output = Result<Response, String>> + 'a,
{
    ...
    let progress_bar = Arc::new(
        ProgressBar::new(file_paths.len().try_into().unwrap())
            ...
            async move {
                match import_function(root_path, current_path, server_url).await {
                    Err(mut import_error) => {
                        import_error
                            .insert_str(0, &format!("{:?} ", current_path.file_name().unwrap()));
                        let mut import_errors_unlocked = import_errors_clone.lock().await;
                        import_errors_unlocked.push(import_error);
                    }
                    Ok(import_success) => {
                        let import_success_readable = format!(
                            "{:?} STATUS {}: {}",
                            current_path.file_name().unwrap(),
                            import_success.status(),
                            import_success.text().await.unwrap_or_default()
                        );
    ...
    errors.extend(
        successes
            .iter()
            .filter(|success| !success.contains("STATUS 200"))
            .map(|x| x.to_owned())
            .collect::<Vec<String>>(),
    );
...
pub async fn send_to_presigned_url(
    target_url: &str,
    file: Vec<u8>,
    ...
    let corrected_url = if target_url.starts_with("http://host.docker.internal:4566/") {
        format!(
            ...
            target_url.trim_start_matches("http://host.docker.internal:4566/")
        )
    } else {
